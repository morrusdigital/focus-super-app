    ...
    <form method="post" action="{{ route('projects.update', $project) }}">
      ...
      <div class="card">
        ...
        <div class="card-body">
          <div class="row g-3">
            <div class="col-md-4">
              ...
              <input class="form-control" name="name" type="text" value="{{ old('name', $project->name) }}" required>
            </div>
            <div class="col-md-4">
              <label class="form-label">Alamat Project</label>
              <input class="form-control" name="address" type="text" value="{{ old('address', $project->address) }}" required>
            </div>
            <div class="col-md-4">
              <label class="form-label">Tanggal Start Kerja</label>
              <input class="form-control" name="start_work_date" type="date" value="{{ old('start_work_date', $project->start_work_date?->format('Y-m-d')) }}">
            </div>
            <div class="col-md-4">
              <label class="form-label">Nilai Kontrak SPK</label>
              <input class="form-control text-end" name="contract_value" type="number" step="0.01" min="0" value="{{ old('contract_value', $project->contract_value) }}" required>
            </div>
            <div class="col-md-4">
              <label class="form-label">PPH</label>
              <select class="form-select" id="use_pph" name="use_pph" required>
                <option value="">-- Pilih --</option>
                <option value="1" @selected((string) old('use_pph', is_null($project->use_pph) ? '' : (int) $project->use_pph) === '1')>Ya</option>
                <option value="0" @selected((string) old('use_pph', is_null($project->use_pph) ? '' : (int) $project->use_pph) === '0')>Tidak</option>
              </select>
            </div>
            <div class="col-md-4 d-none" id="pph_tax_wrapper">
              ...
              <select class="form-select" id="pph_tax_master_id" name="pph_tax_master_id">
                ...
                @foreach ($pphTaxMasters as $taxMaster)
                  <option value="{{ $taxMaster->id }}" @selected((string) old('pph_tax_master_id', $project->pph_tax_master_id) === (string) $taxMaster->id)>
            ...
            <div class="col-md-4">
              ...
              <select class="form-select" id="use_ppn" name="use_ppn" required>
                <option value="">-- Pilih --</option>
                <option value="1" @selected((string) old('use_ppn', is_null($project->use_ppn) ? '' : (int) $project->use_ppn) === '1')>Ya</option>
                <option value="0" @selected((string) old('use_ppn', is_null($project->use_ppn) ? '' : (int) $project->use_ppn) === '0')>Tidak</option>
              </select>
            </div>
            <div class="col-md-4 d-none" id="ppn_tax_wrapper">
              ...
              <select class="form-select" id="ppn_tax_master_id" name="ppn_tax_master_id">
                ...
                @foreach ($ppnTaxMasters as $taxMaster)
                  <option value="{{ $taxMaster->id }}" @selected((string) old('ppn_tax_master_id', $project->ppn_tax_master_id) === (string) $taxMaster->id)>
                    {{ $taxMaster->name }} ({{ number_format($taxMaster->percentage, 2, ',', '.') }}%)
                  </option>
                @endforeach
              </select>
        ...
        <div class="card-footer text-end">
          <a class="btn btn-light" href="{{ route('projects.show', $project) }}">Batal</a>
